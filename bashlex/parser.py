...
def handleNotImplemented(p, type):
    ...
    if len(p) == 2:
        ...
        raise NotImplementedError(
            "type = {%s}, token = {%s}, parts = {%s}" % (type, p[1], p[2])
        )
...
def p_redirection_heredoc(p):
    ...
    output = ast.node(
        kind="word", word=p[len(p) - 1], parts=[], pos=p.lexspan(len(p) - 1)
    )
    if len(p) == 3:
        p[0] = ast.node(
            kind="redirect",
...
def p_function_def(p):
    ...
    p[0] = ast.node(
        kind="function", name=name, body=body, parts=parts, pos=_partsspan(parts)
    )
...
def p_simple_list(p):
    ...
    if (
        len(p) == 2
        and p.lexer._parserstate & flags.parser.CMDSUBST
...
def p_pipeline_command(p):
    ...
    if len(p) == 2:
        if len(p[1]) == 1:
            ...
            p[0] = ast.node(
                kind="pipeline", parts=p[1], pos=(p[1][0].pos[0], p[1][-1].pos[1])
            )
    else:
        # XXX timespec
        ...
        if p[2].kind == "pipeline":
            ...
            p[0] = ast.node(
                kind="pipeline", parts=[node, p[2]], pos=(node.pos[0], p[2].pos[1])
            )
...
def p_error(p):
    ...
    if p.ttype == tokenizer.tokentype.EOF:
        ...
        raise errors.ParsingError(
            "unexpected token %r" % p.value, p.lexer.source, p.lexpos
        )
...
def parsesingle(
    s, strictmode=True, expansionlimit=None, convertpos=False, proceedonerror=False
):
...
def parse(
    ...
    while index < len(s):
        part = _parser(
            s[index:], strictmode=strictmode, proceedonerror=proceedonerror
        ).parse()
...
class _parser(object):
    ...
    def __init__(
        ...
        self.tok = tokenizer.tokenizer(
            s, parserstate=self.parserstate, strictmode=strictmode, **tokenizerargs
        )
