...
def _extractcommandsubst(parserobj, string, sindex, sxcommand=False):
    if string[sindex] == "(":
        ...
        si += 1
        return (
            ast.node(kind="commandsubstitution", command=node, pos=(sindex - 2, si)),
            si,
        )
...
def _paramexpand(parserobj, string, sindex):
    ...
    if c and c in "0123456789$#?-!*@":
        # XXX 7685
        node = ast.node(kind="parameter", value=c, pos=(sindex, zindex + 1))
    elif c == "{":
        # XXX 7863
        ...
        if temp1:
            return (
                ast.node(kind="parameter", value=temp1[1:], pos=(sindex, zindex)),
                zindex,
            )
...
def _expandwordinternal(
    parserobj, wordtoken, qheredocument, qdoublequotes, quoted, isexp
):
    # bash/subst.c L8132
    istring = ""
    parts = []
    ...
    while True:
        ...
        if c in "<>":
            if (
                ...
                or qheredocument
                or qdoublequotes
                or (wordtoken.flags & set([flags.word.DQUOTE, flags.word.NOPROCSUB]))
            ):
                sindex[0] -= 1
        ...
        #     pass
            ...
            if (
                wordtoken.flags & set([flags.word.NOTILDE, flags.word.DQUOTE])
                or (sindex[0] > 0 and not (wordtoken.flags & flags.word.NOTILDE))
                or qdoublequotes
                or qheredocument
            ):
                ...
                stopatcolon = wordtoken.flags & set(
                    [flags.word.ASSIGNRHS, flags.word.ASSIGNMENT, flags.word.TILDEEXP]
                )
                expand = True
                ...
                if i > sindex[0] and expand:
                    node = ast.node(
                        kind="tilde", value=string[sindex[0] : i], pos=(sindex[0], i)
                    )
                    parts.append(node)
