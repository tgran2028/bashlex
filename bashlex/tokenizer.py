...
from bashlex import errors, flags, heredoc, shutils, state, utils
...
class MatchedPairError(errors.ParsingError):
    def __init__(self, startline, message, tokenizer):
        # TODO use startline?
        super(MatchedPairError, self).__init__(
            message, tokenizer.source, tokenizer._shell_input_line_index - 1
        )
...
class token(object):
    ...
    def __eq__(self, other):
        return isinstance(other, token) and (
            ...
            and self.endlexpos == other.endlexpos
...
class tokenizer(object):
    ...
    def __iter__(self):
        while True:
            ...
            if t is eoftoken or (
                self._added_newline and t.lexpos + 1 == len(self._shell_input_line)
            ):
                break
            yield t
    ...
    def token(self):
        ...
        if isinstance(self._current_token, tokentype):
            self._recordpos()
            self._current_token = self._createtoken(
                self._current_token, self._current_token.value
            )

        if (
            self._parserstate & parserflags.EOFTOKEN
            and self._current_token.ttype == self._shell_eof_token
        ):
    ...
    def _readtokenword(self, c):
            ...
            if peek_char == "(" or (c == "$" and peek_char in "{["):
                ...
                if peek_char == "{":
                    ttok = self._parse_matched_pair(
                        cd, "{", "}", firstclose=True, dolbrace=True
                    )
                elif peek_char == "(":
                ...
                try:
                    ttok = self._parse_matched_pair(
                        peek_char, peek_char, peek_char, allowesc=(peek_char == "'")
                    )
                # except MatchedPairError:
                #    return -1
                finally:
        ...
        if (
            ...
            and (
                c in "<>"
                or self._last_read_token.ttype
                in (tokentype.LESS_AND, tokentype.GREATER_AND)
            )
        ...
        tokenword = self._createtoken(
            tokentype.WORD, tokenword, utils.typedset(wordflags)
        )
        if d["dollar_present"]:
        ...
        if self._is_assignment(
            tokenword.value, bool(self._parserstate & parserflags.COMPASSIGN)
        ):
        ...
        elif self._last_read_token.ttype in (
            tokentype.CASE,
            tokentype.SELECT,
            tokentype.FOR,
        ):
    ...
    def _parse_comsub(
        ...
        checkcase = bool(
            parsingcommand
            and (doublequotes is None or doublequotes not in "'\"")
            and not dquote
        )
        ...
        while count:
            c = self._getc(
                doublequotes != "'" and not insidecomment and not passnextchar
            )
            ...
            if (
                not reservedwordok
                and checkcase
                and not insidecomment
                and (_shellmeta(c) or c == "\n")
            ):
                ret += c
            ...
            if not insidecomment and checkcase and c == "<":
                ...
                and not insidecomment
                and c == "#"
                and ((reservedwordok and lexrwlen == 0) or insideword or lexwlen == 0)
            ):
                insidecomment = True
            ...
            if _shellquote(c):
                ...
                try:
                    if wasdollar and c == "'":
                        nestret = self._parse_matched_pair(
                            c, c, c, allowesc=True, dquote=True
                        )
                    else:
            ...
            # check for $(), $[], or ${} inside command substitution
                ...
                if not insidecase and open == c:
                    count -= 1
                if c == "(":
                    nestret = self._parse_comsub(
                        None, "(", ")", parsingcommand=True, dquote=False
                    )
                elif c == "{":
                    nestret = self._parse_matched_pair(
                        None, "{", "}", firstclose=True, dolbrace=True, dquote=True
                    )
                elif c == "[":
    ...
    def _parse_matched_pair(
        ...
        def handledollarword():
            ...
            if c == "(":
                return self._parse_comsub(
                    None, "(", ")", parsingcommand=True, dquote=False
                )
            elif c == "{":
                return self._parse_matched_pair(
                    None, "{", "}", firstclose=True, dquote=rdquote, dolbrace=True
                )
            elif c == "[":
    ...
    def _command_token_position(self, token):
        return (
            token.ttype == tokentype.ASSIGNMENT_WORD
            or self._parserstate & parserflags.REDIRLIST
            or (
                token.ttype
                not in (
                    tokentype.SEMI_SEMI,
                ...
                and self._reserved_word_acceptable(token)
            )
        )

    def _assignment_acceptable(self, token):
        return (
            self._command_token_position(token)
            and not self._parserstate & parserflags.CASEPAT
        )
    ...
    def _reserved_word_acceptable(self, tok):
        ...
        if (
            self._last_read_token.ttype == tokentype.WORD
            and self._token_before_that.ttype == tokentype.FUNCTION
        ):
            return True
    ...
    def _ungetc(self, c):
        if (
            self._shell_input_line
            and self._shell_input_line_index
    ...
    def _specialcasetokens(self, tokstr):
        if (
            self._last_read_token.ttype == tokentype.WORD
            and self._token_before_that.ttype
            in (tokentype.FOR, tokentype.CASE, tokentype.SELECT)
            and tokstr == "in"
        ):
        ...
        if (
            self._last_read_token.ttype == tokentype.WORD
            and self._token_before_that.ttype in (tokentype.FOR, tokentype.SELECT)
            and tokstr == "do"
        ):
            return tokentype.DO
