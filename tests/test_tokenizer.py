...
class test_tokenizer(unittest.TestCase):
    ...
    def test_simple(self):
        s = "a b"
        self.assertTokens(s, [t(tt.WORD, "a", [0, 1]), t(tt.WORD, "b", [2, 3])])
    ...
    def test_meta(self):
        ...
        s = "<&-"
        self.assertTokens(s, [t(tt.LESS_AND, "<&", [0, 2]), t(tt.DASH, "-", [2, 3])])
    ...
    def test_shellquote(self):
        s = '"foo"'
        self.assertTokens(s, [t(tt.WORD, '"foo"', [0, 5], set([flags.word.QUOTED]))])

        s = "\"foo\"bar'baz'"
        self.assertTokens(s, [t(tt.WORD, s, [0, len(s)], set([flags.word.QUOTED]))])
    ...
    def test_parameter_expansion(self):
        ...
        s = "a $$"
        self.assertTokens(
            s, [t(tt.WORD, "a", [0, 1]), t(tt.WORD, "$$", [2, 4], hasdollarset)]
        )

    def test_comsub(self):
        s = "a $(b)"
        self.assertTokens(
            s, [t(tt.WORD, "a", [0, 1]), t(tt.WORD, "$(b)", [2, 6], hasdollarset)]
        )
        ...
        s = "a $(! b)"
        self.assertTokens(
            s, [t(tt.WORD, "a", [0, 1]), t(tt.WORD, "$(! b)", [2, 8], hasdollarset)]
        )
        ...
        s = "$(case ;; esac)"
        self.assertTokens(s, [t(tt.WORD, "$(case ;; esac)", [0, 15], hasdollarset)])
        ...
        s = "$(a\\b)"
        self.assertTokens(s, [t(tt.WORD, "$(a\\b)", [0, len(s)], hasdollarset)])
    ...
    def test_plus_at_end_of_word(self):
        s = "a+ b"
        self.assertTokens(s, [t(tt.WORD, "a+", [0, 2]), t(tt.WORD, "b", [3, 4])])
    ...
    def test_foo(self):
        s = "c)"
        self.assertTokens(s, [t(tt.WORD, "c", [0, 1]), t(tt.RIGHT_PAREN, ")", [1, 2])])

    def test_redirections(self):
        s = "1>"
        self.assertTokens(s, [t(tt.NUMBER, 1, [0, 1]), t(tt.GREATER, ">", [1, 2])])
        s = "$<$(b)"
        self.assertTokens(
            s,
            [
    ...
    def test_escape_error(self):
        ...
        self.assertRaisesRegex(
            errors.ParsingError, "No escaped character.*position 2", tokenize, s
        )

    def test_tokenize(self):
        s = "bar -x"
        self.assertTokens(s, [t(tt.WORD, "bar", [0, 3]), t(tt.WORD, "-x", [4, 6])])
    ...
    def test_escaped_newline(self):
        s = """a \\\nb"""
        self.assertTokens(s, [t(tt.WORD, "a", [0, 1]), t(tt.WORD, "b", [4, 5])])
