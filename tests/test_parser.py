...
from bashlex import ast, errors, flags, parser, state
...
def redirectnode(s, input, type, output, heredoc=None):
    return ast.node(
        kind="redirect", input=input, type=type, output=output, heredoc=heredoc, s=s
    )
...
class test_parser(unittest.TestCase):
    ...
    def assertASTEquals(self, s, expected, **parserargs):
        ...
        self.assertTrue(
            len(results) == 1, "expected one ast from parse(), " "got %d" % len(results)
        )
        result = results[0]
    ...
    def assertASTsEquals(self, s, expectedlist, **parserargs):
        results = parse(s, **parserargs)
        self.assertEqual(len(results), len(expectedlist), "mismatch on ASTs length")
    ...
    def test_command(self):
        ...
        self.assertASTEquals(
            s, commandnode(s, wordnode("a"), wordnode("b"), wordnode("c"))
        )

        s = 'a b "c"'
        self.assertASTEquals(
            s, commandnode(s, wordnode("a"), wordnode("b"), wordnode("c", '"c"'))
        )
    ...
    def test_multiline(self):
        s = "a\nb"
        self.assertASTsEquals(
            s, [commandnode("a", wordnode("a")), commandnode("b", wordnode("b"))]
        )
        ...
        self.assertASTEquals(
            ...
            commandnode(
                ...
                wordnode(
                    ...
                    "`b`",
                    [
                        comsubnode("`b`", commandnode("b", wordnode("b"))),
                    ],
                ),
                wordnode(
                    "`c`",
                    '"`c`"',
                    [
                        comsubnode("`c`", commandnode("c", wordnode("c"))),
                    ],
                ),
                wordnode("`c`", "'`c`'"),
            ),
    ...
    def test_redirection_input(self):
        ...
        self.assertASTEquals(
            s,
            commandnode(s, wordnode("a"), redirectnode("<f", None, "<", wordnode("f"))),
        )

        s = "a <1"
        self.assertASTEquals(
            s,
            commandnode(s, wordnode("a"), redirectnode("<1", None, "<", wordnode("1"))),
        )

        s = "a 1<f"
        self.assertASTEquals(
            s, commandnode(s, wordnode("a"), redirectnode("1<f", 1, "<", wordnode("f")))
        )
        ...
        s = "a 0<&3"
        self.assertASTEquals(
            s, commandnode(s, wordnode("a"), redirectnode("0<&3", 0, "<&", 3))
        )
        ...
        self.assertASTEquals(
            ...
            pipelinenode(
                ...
                compoundnode(
                    ...
                    reservedwordnode(")", ")"),
                    redirects=[
                        redirectnode("> /dev/null", None, ">", wordnode("/dev/null"))
                    ],
                ),
            ),
        )
    ...
    def test_group(self):
        ...
        s = "echo {}"
        self.assertASTEquals(s, commandnode(s, wordnode("echo"), wordnode("{}")))

        # reserved word at beginning isn't reserved if quoted
        s = "'{' foo"
        self.assertASTEquals(s, commandnode(s, wordnode("{", "'{'"), wordnode("foo")))
        ...
        self.assertASTEquals(
            ...
            listnode(
                ...
                compoundnode(
                    ...
                    reservedwordnode("{", "{"),
                    listnode(
                        "b;", commandnode("b", wordnode("b")), operatornode(";", ";")
                    ),
                    reservedwordnode("}", "}"),
                ),
            ),
    ...
    def test_invalid_control(self):
        ...
        self.assertRaisesRegex(
            errors.ParsingError, "unexpected token '|'.*position 3", parse, s
        )

    def test_invalid_redirect(self):
        s = "a 2>"
        self.assertRaisesRegex(
            errors.ParsingError, r"unexpected token '\\n'.*position 4", parse, s
        )
    ...
    def test_malformed_if(self):
        s = "if foo; bar; fi"
        self.assertRaisesRegex(
            errors.ParsingError, "unexpected token 'fi'.*position 13", parse, s
        )

        s = "if foo; then bar;"
        self.assertRaisesRegex(
            errors.ParsingError, "unexpected EOF.*position 17", parse, s
        )

        s = "if foo; then bar; elif baz; fi"
        self.assertRaisesRegex(
            errors.ParsingError, "unexpected token 'fi'.*position 28", parse, s
        )
    ...
    def test_word_expansion(self):
        ...
        s = '"a\'b"'
        self.assertASTEquals(s, commandnode(s, wordnode("a'b", s)))

        s = "a\"b\"'c'd"
        self.assertASTEquals(s, commandnode(s, wordnode("abcd", s)))
    ...
    def test_escape_not_part_of_word(self):
        s = "a \\;"
        self.assertASTEquals(s, commandnode(s, wordnode("a"), wordnode(";", "\\;")))
    ...
    def test_heredoc_spec(self):
        ...
        self.assertASTEquals(
            s,
            commandnode(
                s, wordnode("a", "a"), redirectnode("<<-b", None, "<<-", wordnode("b"))
            ),
            strictmode=False,
        )
...
%s"""
        ...
        self.assertASTEquals(
            ...
            commandnode(
                ...
                wordnode("a"),
                redirectnode(
                    "<<EOF\n%s" % doc, None, "<<", wordnode("EOF"), heredocnode(doc)
                ),
            ),
        )
        ...
        s = "a <<EOF\nb"
        self.assertRaisesRegex(
            errors.ParsingError, "delimited by end-of-file \\(wanted 'EOF'", parse, s
        )
        ...
        self.assertASTEquals(
            ...
            compoundnode(
                ...
                fornode(
                    ...
                    listnode(
                        "d;", commandnode("d", wordnode("d")), operatornode(";", ";")
                    ),
        ...
        self.assertASTEquals(
            ...
            compoundnode(
                ...
                whilenode(
                    ...
                    listnode(
                        "a;", commandnode("a", wordnode("a")), operatornode(";", ";")
                    ),
                    reservedwordnode("do", "do"),
                    listnode(
                        "b;", commandnode("b", wordnode("b")), operatornode(";", ";")
    ...
    def test_command_arithmetic(self):
        self.assertRaisesRegex(
            NotImplementedError, "arithmetic expansion", parse, 'a "$((2 + 2))"'
        )
    ...
    def test_function_parenthesis_optional(self):
        ...
        self.assertASTEquals(
            s,
            functionnode(
                s, name, body, reservedwordnode("function", "function"), name, body
            ),
        )

        s = "a { b; }"
        self.assertRaisesRegex(errors.ParsingError, "unexpected token '}'.*7", parse, s)
        ...
        self.assertASTEquals(
            ...
            compoundnode(
                ...
                fornode(
                    ...
                    wordnode("/asdf/*", "/asdf/*"),
                    reservedwordnode("do", "do"),
                    commandnode("echo hook", wordnode("echo"), wordnode("hook")),
                    reservedwordnode("done", "done"),
                ),
            ),
        )
    ...
    def test_ending_newlines(self):
        ...
        self.assertASTEquals(
            s,
            commandnode(
                s.strip(), wordnode("echo"), wordnode("hello"), wordnode("world")
            ),
        )
    ...
    def test_unimplemented(self):
        ...
        self.assertASTEquals(
            s,
            unimplementednode(
                s, reservedwordnode("coproc", "coproc"), wordnode("echo", "echo")
            ),
            proceedonerror=True,
        )
